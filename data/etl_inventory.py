...
def validate(products, inventory):
    ...
    if (products["unit_cost"] > products["unit_price"]).any():
        issues.append("products: unit_cost exceeds unit_price on some rows (margin < 0)")
    if issues:
...
def clean_inventory(inventory: pd.DataFrame) -> pd.DataFrame:
    df = inventory.drop_duplicates(subset=["product_id", "warehouse_region", "snapshot_date"]).copy()
    df["last_restock_date"] = pd.to_datetime(df["last_restock_date"], errors="coerce")
    df["snapshot_date"] = pd.to_datetime(df["snapshot_date"], errors="coerce")
    df["stock_on_hand"] = df["stock_on_hand"].clip(lower=0)  # negative stock is invalid
    return df
